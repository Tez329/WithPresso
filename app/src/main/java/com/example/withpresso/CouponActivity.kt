...
import com.example.withpresso.service.CouponList
...
class CouponActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        couponService.requestCoupon(user_asin).enqueue(object :Callback<CouponList> {
            /* 통신 실패 */
            override fun onFailure(call: Call<CouponList>, t: Throwable) {
                Log.e("coupon list request", "failed")
                toast("쿠폰 받아오기 실패")
            }

            /* 통신 성공 */
            override fun onResponse(call: Call<CouponList>, response: Response<CouponList>) {
                Log.d("coupon list request", "success")

                val result = response.body()
                result?.let {
                    coupon_recycler.adapter = CouponAdapter(
                        this@CouponActivity, result.coupon_list as ArrayList<Coupon>
                    )
                }
            }
        })
